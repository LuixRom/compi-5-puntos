...
export function AutomatonVisualizer({ automaton, title, description }: AutomatonVisualizerProps) {
  ...
  if (!automaton) {
    return (
      <Card>
        <CardContent className="text-center py-12 text-muted-foreground">No hay datos del autómata</CardContent>
      </Card>
    );
  }

  const names = Object.keys(automaton.states);
  let trans = 0; names.forEach(n => Object.values(automaton.states[n].transitions).forEach(ts => (trans += ts.length)));
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Badge variant="outline">{names.length} estados</Badge>
            <Badge variant="outline">{trans} transiciones</Badge>
          </div>
        </CardTitle>
      ...
      <CardContent>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {/* Descarga de lo visible (original) */}
            ...
            {/* Descarga completa */}
            <Button variant="outline" size="sm" onClick={() => handleDownloadFull("png")}>
              <Download className="w-4 h-4 mr-1" />
              Descargar PNG (completo)
            </Button>
            <Button variant="outline" size="sm" onClick={() => handleDownloadFull("pdf")}>
              <Download className="w-4 h-4 mr-1" />
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            <Badge>Alfabeto: {automaton.alphabet.join(", ")}</Badge>
            <Badge variant="secondary">Inicial: {automaton.start_state}</Badge>
          </div>

          {/* Visor con zoom/pan */}
          <div ref={wrapperRef} className="border rounded-lg bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 overflow-hidden h-[32rem]">
            <canvas ref={canvasRef} className="rounded-lg w-full h-full block cursor-move" />
          </div>
          ...
          <div className="flex gap-4 flex-wrap text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 rounded-full border-2 border-green-500 bg-white relative">
                <div className="absolute inset-0.5 rounded-full border border-green-500" />
              </div>
              <span>Estado final</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 rounded border-2 border-slate-500 bg-white" />
              <span>Estado DFA (rectángulo)</span>
            </div>
          </div>

          {/* === INFO ABAJO (Estados y Transiciones) === */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <h4 className="font-semibold mb-2">Estados ({names.length}):</h4>
              <div className="space-y-1 max-h-64 overflow-y-auto">
                {names.map(n => {
                  const s = automaton.states[n];
                  return (
                    <div key={n} className="flex items-start gap-2 p-2 hover:bg-muted/50 rounded">
                      <span
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium">
                          {n}
                          <span className="text-xs opacity-70 ml-2">({s.shape === "rect" ? "rectángulo" : "óvalo"})</span>
                        </div>
                        {s.label && (
                          <pre className="text-xs opacity-80 whitespace-pre-wrap break-words">{s.label.replace(/\\n/g, "\n")}</pre>
                        )}
                      </div>
                    </div>
                  );
            ...
            <div>
              <h4 className="font-semibold mb-2">Transiciones ({trans}):</h4>
              <div className="space-y-1 max-h-64 overflow-y-auto font-mono text-xs">
                {names.flatMap(from =>
                  Object.entries(automaton.states[from].transitions).flatMap(([sym, tos]) =>
                    tos.map((to, i) => (
                      <div key={`${from}-${sym}-${to}-${i}`} className="p-1 hover:bg-muted/50 rounded">
                        δ({from}, <span className="text-primary font-semibold">{sym}</span>) = {to}
                      </div>
                    ))
                  )
                )}
